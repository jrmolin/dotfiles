...
export DEV_PREFIX=/opt/toolchains

# Don't forget to adjust this to your NDK path
export ANDROID_NDK=${DEV_PREFIX}/android-ndk-r9d/
...
#./configure --host=$(uname -m)-linux-gnu --target=${CROSS_COMPILE} --with-sysroot=${SYSROOT} --prefix=${PREFIX} "$@"
${CONFIGUREPATH}/configure --host=$(uname -m)-linux-gnu --build=arm --with-sysroot=${SYSROOT} --prefix=${PREFIX} "$@"
